...
import { inspectSandbox, listHistory, preflight } from "@/lib/admin/sandbox-publish";
...
export default async function PublishPage({ searchParams }: { searchParams: Promise<{ ok?: string; err?: string }> }) {
  ...
  const errors = await preflight(ws.doc);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="a-muted mt-2 max-w-2xl">Publicar aqui grava só no <strong>sandbox local</strong> (<code>data/admin-dev/published</code>). A loja de produção não é tocada. A loja local só mostra o resultado quando iniciada com <code>npm run cms:dev</code>.</p>
      </div>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="a-card p-5">
          ...
          <form action={publishAction} className="mt-5 space-y-3">
            ...
            <button type="submit" className="a-btn" disabled={errors.length > 0 || changes.length === 0}>Publicar no sandbox local</button>
        ...
        <div className="a-card p-5">
          <p className="a-h2">Estado do sandbox</p>
          ...
          <p className="a-muted mt-3 text-[0.8125rem]">Usa o mesmo protocolo de duas fases da produção (registro → arquivo atômico → promoção → invalidação de cache) e o mesmo reconciliador, com um registro em JSON no lugar do Postgres.</p>
        </div>
      </section>

      <section className="a-card overflow-x-auto" aria-label="Histórico">
        <table className="a-table">
          <caption className="sr-only">Histórico de publicações do sandbox</caption>

...
import type { MediaChoice, RemoveResult, UploadResult } from "./media/types";

/** The screens' view of the media library: whichever store the platform selected (local sandbox files, or R2 + Postgres). */
...
export const resolveMedia = (assetIds: Iterable<string>) => platform().media.resolve(assetIds);

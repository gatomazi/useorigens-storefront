...
/**
 * Production media: processed WebP variants in R2 (`media/<sha256>/<width>.webp`, immutable, content-addressed), metadata in Postgres
 * (`media_asset`), public URLs under the media origin (`MEDIA_PUBLIC_BASE_URL`, e.g. https://media.useorigens.com.br). Only processed
 * output is ever stored: the uploaded original is discarded (privacy: no EXIF/GPS survives, and no untrusted bytes are ever served).
 *
 ...
 * is a deliberate, manual operation (docs/admin/cms-v1-round6.md).
 */
type Row = { id: string; label: string | null; width: number; height: number; variants: { w: number; key: string }[] };

export function r2MediaStore(deps: { db: Db; objects: ObjectStore | null; publicBase: string | null }): MediaStore {
  const { db, objects, publicBase } = deps;
  const url = (key: string) => `${publicBase!.replace(/\/$/, "")}/${key}`;
  const thumb = (row: Row) => (row.variants.find((v) => v.w >= 640) ?? row.variants.at(-1)!).key;
  const choiceOf = (row: Row): MediaChoice => ({ assetId: `upload:${row.id}`, label: row.label ?? "imagem", src: url(thumb(row)), width: row.width, height: row.height, kind: "upload" });

  return {
    canUpload: Boolean(objects && publicBase),
    async list() {
      const rows = publicBase
    ...
    async resolve(assetIds) {
      ...
      if (uploads.length > 0 && publicBase) {
    ...
    async save(bytes, originalName, actorId) {
      if (!objects || !publicBase) return { ok: false, error: "o envio de imagens não está configurado neste ambiente (R2)" };

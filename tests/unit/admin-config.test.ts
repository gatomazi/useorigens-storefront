...
describe("admin mode decision", () => {
  test("given a production process with the complete variable set, when decided, then the admin is on, normalised, and Google is the issuer", () => {
    const c = adminConfig(FULL);
    expect(c).toMatchObject({ mode: "prod", adminHost: "admin.useorigens.com.br", adminOrigin: "https://admin.useorigens.com.br", ownerEmail: "dono@example.com", oidcIssuer: "https://accounts.google.com" });
  });

  test("given any single required variable missing, when decided, then the admin is OFF and names what is missing", () => {
    for (const name of ["ADMIN_HOST", "ADMIN_OWNER_EMAIL", "GOOGLE_OAUTH_CLIENT_ID", "GOOGLE_OAUTH_CLIENT_SECRET", "ADMIN_SESSION_SECRET", "DATABASE_URL"]) {
  ...
  test("given an OIDC issuer override, when it is not Google or a loopback http URL, then the admin stays OFF", () => {
